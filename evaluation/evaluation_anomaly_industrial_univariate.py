...
from utils.tools import get_time_series_dataset, get_sliding_window_matrix, \
    get_settings, check_setup, lazy_check_setup, label_settings
from utils.manage_file import get_files
...
settings_file = '/Users/delbu/Projects/PycharmProjects/SBDIO/data/setting/settings_Ricette.CSV'
...
# This is valid only for multi variate time series anomaly detection
features_list = [
    'TEMP_E2',
    'TEMP_E3',
    'TEMP_E4',
...
def main():
    ...
    for file in files:
        ...
        # Check if the setting start overlap with file time interval
    ...
    print('Number of time series with setup: {}'.format(len(setup_files)))
    ...
    for k in y_pred_single.keys():
        for col in y_pred_single[k].keys():
            i = 0
            for pred, true in zip(y_pred_single[k][col], y_true_single[k][col]):
                y_pred.append(pred)
                y_true.append(true)
                cols.append(col)
